            ...
            if !AppConfig.isAPIConfigured {
                HStack(alignment: .top, spacing: 8) {
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Chưa cấu hình API")
                            .font(.footnote.weight(.black))
                        Text("Bản build này thiếu MHM_API_BASE_URL trong Info.plist. Cập nhật Config.xcconfig rồi build lại.")
                ...
                .frame(maxWidth: .infinity, alignment: .leading)
                .padding(12)
                .background(Color(hex: 0x78350F).opacity(0.45), in: RoundedRectangle(cornerRadius: 14))
            }
